...
public:
	...
	int  nread;
	ReadBuffer() {
		buf = (int*)malloc(sizeInt*bufferSize);
		if(buf == NULL)
		{
	...
	void open(FILE *infile_t, int offset) {
		...
		infile = infile_t;
		rewind(infile);
		ret = fseeko64(infile, offset*sizeInt, SEEK_SET);
		nread = fread(buf, sizeInt, intsPerBlock, infile);
		ptr = 0;
		isend = false;
	};
...
public:
	WriteBuffer() {
		buf = (int*)malloc(sizeInt*bufferSize); // change WriteBuffer
		if(buf == NULL)
		{
      printf("[%d] malloc fail!\n", __LINE__);
		  exit(1);
	...
	void open(FILE *outfile_t, int offset) {
		...
		ptr = 0;
		rewind(outfile);
		fseeko64(outfile, offset*sizeInt, SEEK_SET);
	}
